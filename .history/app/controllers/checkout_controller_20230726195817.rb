class CheckoutController < ApplicationController
  ...
  def success
     ...
     customer_province = JSON.parse(metadata['customer_province'])['province']

    # Fetch the HST, GST, and PST values from the provinces table using the address province
    province = Province.find_by(name: customer_province)
    @gst_rate = province.GST.to_f
    @pst_rate = province.PST.to_f
    @hst_rate = province.HST.to_f
     ...
     @order = Order.create!(
       ...
       GST: @gst_r, # Change gst_rate to the appropriate value (e.g., 0.05 for 5%)
