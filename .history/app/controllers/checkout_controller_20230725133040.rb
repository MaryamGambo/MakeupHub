class CheckoutController < ApplicationController
  ...
  def calculate_taxes(subtotal)
    ...
    if customer_signed_in? && (current_customer.primary_address || current_customer.alt_address)
      ...
      province_id = session[:guest_address][:province].to_i
      puts "Using province from session: #{province_id}"
    end

    province = Province.find_by(id: province_id)
    puts "Found Province: #{province.inspect}" if province
    ...
    puts "GST Rate: #{gst_rate}"
  puts "PST Rate: #{pst_rate}"
  puts "HST Rate: #{hst_rate}"
