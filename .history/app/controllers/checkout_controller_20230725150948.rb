class CheckoutController < ApplicationController
  ...
  def create
    ...
    taxes_item = {
      price_data: {
        ...
        product_data: {
          ...
          images: [], # You can add an image for the taxes if needed
        },
    ...
    session = Stripe::Checkout::Session.create({
      ...
      metadata: {
        # Add any metadata you want to save with the session
        # For example, you can store order details, customer information, etc.
        # The data will be accessible in the `success` action when processing the payment.
        order_id: 'YOUR_ORDER_ID', # Replace with your order ID or any unique identifier for the order
        customer_id: current_customer.id, # Replace with the customer's ID if you have a signed-in customer
      }
    })
